...
**Дата:** 24.04.2026
**Актуальная версия:** v2.26 (Сессия 16 закрыта, в патче zip к деплою)
**Статус:** билд чистый, 103 страницы, 16 сек, на webnostra.pro (после push)
**Следующая задача:** не определена - на выбор: блог-кластер "Инфобиз", "Туризм/Медицина", локальное SEO, публичный демо-дашборд
...
### Сессия 16 (готово, v2.26) - технические хвосты: фото Магоша и чистка "80 продаж"

Небольшой патч после большой работы над кейсами в Сессиях 15.1 и 15.2. Задача - закрыть два висящих технических долга которые давно раздражали и требовали внимания.

**1. Фото Магоша для AuthorByline**

Заменено фото в public/team/magosh.jpg. Было: 1920x2560 портретный снимок в кепке в Starbucks, непропорциональный для круглой аватарки. Стало: квадрат 800x800 с tight-кропом на лице (очки, сосредоточенный взгляд, образ работающего за MacBook). Оптимизировано под web - 112КБ прогрессивный JPEG.

Процесс обработки:
- Исходник - HEIC с iPhone (3024x4032)
- ImageMagick convert через libheif в PNG
- Кроп 1600x1600 с центром на лице (+712+750 от левого верха)
- Resize 800x800 с strip метаданных, quality 90, interlace Plane

Использование в коде не менялось - все места подхватывают новый файл:
- src/pages/blog/[slug].astro (17 статей)
- src/pages/portfolio/[slug].astro (53 кейса)
- src/components/seo/AuthorByline.astro (default value)
- src/content/team.json (карточка команды)
- src/components/sections/Team.astro (раздел на главной)

**2. Чистка "80 продаж" Beyond Taylor**

В CHAT-CONTEXT давно висела пометка что в /services/sales-support/ фигурирует цифра "80 продаж" которую Магош не подтвердил и которой нет в PDF Beyond Taylor. Проверил - в PDF есть только 4.2М₽ бюджет, 2,444 заявок, 1,719₽ CPL, 12.5М₽ выручка. "80 продаж" было расчетной цифрой (12.5М/150К ≈ 83), которая выглядела конкретно но не была проверенной.

Убрано 10 упоминаний в 5 файлах:
- src/content/cases/beyond-taylor.mdx - shortDescription
- src/content/faq.json - один ответ
- src/pages/industries/edtech.astro - 5 мест: флагман-плашка, описание премиум-сегмента, блок "Квиз+КЭВ", заголовок inline-кейса, абзац описания
- src/pages/industries/index.astro - одна строка
- src/pages/services/sales-support.astro - 2 места: label и h3 inline-кейса, а также в case-numbers "80 продаж" заменено на "1,719₽ CPL"

Замена формулировок разнообразная (чтобы не было одинаковых фраз на всех страницах):
- "2,444 заявок → 12.5М₽ выручки"
- "4.2М₽ бюджета → 12.5М₽ выручки (ROMI 3x)"
- "2,444 заявки за 9 месяцев при бюджете 4.2М₽ и 12.5М₽ выручки"

**3. llms-full.txt аудит**

Проверил покрытие всех 53 MDX-кейсов в llms-full.txt. Оказалось что файл уже обновлен - все 53 кейса упомянуты с цифрами (Алматы, Астана, Минск, Прага, Дубай ортодонтия, Яхты Пхукет, Санаторий Пирогова, и т.д. - все на месте). Плюс 17 блог-URL с описаниями. Файл в хорошем состоянии, правок не требует.

**Что НЕ вошло в Сессию 16 (на следующие сессии):**

- **Privacy и Offer юристу** - вне моей зоны, организует Магош
- **Яндекс.Вебмастер https** - делается в админке Яндекса, инструкция ниже
- **Rich Results Test** - проверка браузером после деплоя, инструкция ниже
- **Первый замер Prompt Win Rate** - 10 запросов в ChatGPT/Perplexity/Яндекс Нейро вручную
- **Person schema sameAs Магоша** - решили не менять (только Telegram/Instagram)
- **country-houses и phuket-developer** - решили оставить featured без выведения на главную
- **PNG-иконки для soссетей в футере** - не запрашивалось, оставлено

**Инструкции для Магоша (ручные шаги):**

1. **Rich Results Test после деплоя v2.26:**
   - Открыть https://search.google.com/test/rich-results
   - Ввести https://webnostra.pro/portfolio/luding/ - проверить Article schema
   - Ввести https://webnostra.pro/portfolio/ - проверить ItemList schema
   - Ввести https://webnostra.pro/authors/magosh/ - проверить Person schema
   - Скриншотить результаты если есть ошибки - пришлю решение

2. **Яндекс.Вебмастер https:**
   - https://webmaster.yandex.ru/ → Сайт webnostra.pro
   - Настройки индексирования → Настройки сайта
   - Главное зеркало → проверить что стоит с https://
   - Индексирование → Переобход страниц → отправить обновленный /portfolio/ хаб и добавленные /portfolio/<slug>/ 15.1+15.2 страницы (можно 20 URL за раз)

3. **Google Search Console после деплоя:**
   - https://search.google.com/search-console/
   - В GSC: Settings → Verification → проверить что верификация сайта активна
   - Sitemaps → отправить sitemap-index.xml (если еще не отправлен)
   - URL Inspection → проверить /portfolio/luding/ и /blog/luding-vitrina-alkogolya-210m/

4. **Первый замер Prompt Win Rate:**
   - Зайти последовательно в: chat.openai.com, perplexity.ai, ya.ru (Нейро)
   - Задать 10 запросов из списка (в блоке Prompt Win Rate мониторинга ниже)
   - Для каждого записать: упомянута ли WEBNOSTRA (да/нет), позиция в ответе (1/2/3/не названа), цитата если есть
   - Создать файл Google Sheets "GEO Monitoring" со столбцами: Date, Platform, Query, Mentioned (Y/N), Position, Citation
   - Стартовая планка 10-20% - это значит 1-2 запроса из 10 на каждой платформе упомянут WEBNOSTRA
   - Цель через 3 месяца - 40-55% (4-6 из 10)
   - Повторять замер раз в 2 недели в одной и той же среде

**Уроки Сессии 16:**

1. Кропать HEIC с iPhone в квадратные аватарки через ImageMagick (convert через libheif) работает стабильно. Для tight-crop нужно предварительно посмотреть фото в preview размере, прикинуть координаты лица, потом кропнуть 1600x1600 и даунсайзить. Вес 112КБ на 800x800 - оптимально для web.

2. В CHAT-CONTEXT бывают устаревшие пометки которые на самом деле уже не актуальны. Перед тем как править - надо делать sanity-check (grep, ls, open file). Ситуация с llms-full.txt показала: файл уже обновлен в какой-то прошлой сессии, но в CHAT-CONTEXT это не зафиксировано.

3. Grep по кириллице с lowercase/uppercase чувствителен к регистру и не всегда ловит слова в правильной форме - при аудите текста на русском лучше использовать `grep -i` и проверять через разные формы слова. В Сессии 16 потерял 10 минут думая что 7 кейсов пропущены, на самом деле все были в файле.

4. "80 продаж" как цифра - хороший урок: любая расчетная цифра которая выглядит конкретно, но не взята из оригинала кейса - опасна. Всегда ставить в текст только проверяемые числа из PDF.
...
- ~~Фото Магоша для AuthorByline~~ закрыто в Сессии 16 (новое фото 800x800)
- ~~Beyond Taylor "80 продаж" непроверенная цифра~~ закрыто в Сессии 16 (10 упоминаний вычищено)
- ~~llms-full.txt аудит~~ проведен в Сессии 16 - файл в хорошем состоянии, правок не требует

**Активные:**

1. **Privacy и Offer** - перед длинной дистанцией показать юристу Магоша.

2. **country-houses и phuket-developer** - featured, но не попадают на главную из-за старых publishedAt. Решили оставить как есть (не все featured обязаны попадать на главную).

3. **Яндекс.Вебмастер https** - указать главное зеркало с https://, переобойти sitemap для 30 новых /portfolio/*. Инструкция в описании Сессии 16.

4. **Rich Results Test** - после деплоя v2.26 проверить Article, ItemList, Person schema. Инструкция в описании Сессии 16.

5. **GEO мониторинг Prompt Win Rate** - первый замер 10 запросов в ChatGPT/Perplexity/Яндекс Нейро. Инструкция и список запросов в описании Сессии 16. Запросы для начала:
...
6. **Цифры в листикле "Топ-10 агентств Пхукет"** - часть агентств описаны по публичным данным. Если Магош знает реальные ценники конкурентов точнее - править соответствующий блок в .mdx.

7. **Person schema sameAs Магоша** - решили не менять (только Telegram и Instagram остаются). Если появятся LinkedIn, VC, Habr - править в src/lib/seo-geo.ts → TEAM_PERSONS.magosh.sameAs.

8. **Варианты следующих направлений (на выбор):**
    - **Блог-кластер "Инфобиз"** - 7 статей по материалам Beyond Taylor, Algoritmika, Beauty School, Aifluent, Telegram-школа, English-школа, Sommelier, Driving. После Сессии 15.2 в Astro-кейсах есть 9 edtech-кейсов для мощной перелинковки
    - **Блог-кластер "Туризм/Медицина"** - 5-7 статей. Есть 14 кейсов в этих нишах для опоры (7 tourism, 7 medical)
    - **Локальное SEO** - страницы услуг по городам (/services/meta-ads/moscow/ и т.д.), риск cannibalization если не продумать архитектуру
    - **Публичный демо-дашборд /live/** - из давних планов, обфусцированные реальные данные клиента. Большая задача.
    - **Обновление industries.json** под реальные счетчики MDX (опционально, не критично)
